...
[v0.3.0 更新说明](docs/RELEASE_v0.3.0.md) · [快速开始](#快速开始) · [功能与截图](#功能与截图) · [常见问题](#常见问题) · [许可与使用边界](#许可与使用边界)
...
本地资源功能可离线使用；在线卡面、公开台词及首次下载语音识别模型仍需联网。以下为源码运行方式。
...
想探索不属于某张卡的音乐、棋盘环境声或界面音效，请进入“声音资料库”，先点击右上角 **建立完整资源索引**。扫描耗时取决于资源规模和磁盘速度，可暂停后继续。首次连接会弹窗介绍索引，可暂不建立；卡牌语音采用按块读取，无需先扫描全部资源。性能实现与验证见 [性能说明](docs/PERFORMANCE.md)。
...
**可以完全离线吗？** 免安装版解压后（源码版安装好依赖后），本地资源解析、原画、试听与导出可离线运行。完整卡面、公开台词查询和首次下载识别模型需要联网；可在设置中关闭公开台词补充和自动识别，并避免打开在线完整卡面。已有模型的识别可离线运行。
...
纯单元测试不需要游戏；`tools/` 中的本地集成／GUI 验证需要炉石资源和可用桌面。v0.3.0 发布验证时，116 项测试通过。
