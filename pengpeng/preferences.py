...
def validate_views(value):
    ...
    result = {}
    for view in ('cards', 'heroes', 'battlegrounds', 'audio', 'effects'):
        item = value.get(view)
        ...
        result[view] = {
            ...
            'subgroup': str(item.get('subgroup', ''))[:100],
            ...
            'filters': {k: str(v)[:100] for k, v in filters.items()
                        if k in {'set', 'format', 'class', 'rarity', 'cost', 'type',
                                 'hero_group', 'collectible', 'battlegrounds', 'race', 'keyword', 'tier', 'bg_pool', 'new'} and isinstance(v, (str, int))}
        ...
        if view in ('cards', 'heroes', 'battlegrounds'):
            ...
            result[view]['display'] = {
                ...
                'size': max(140, min(280, size)) if type(size) is int else 180}
    return result
