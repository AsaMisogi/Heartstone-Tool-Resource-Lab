...
if __name__ == '__main__':
    ...
    output=Path('.cache/qa-09');output.mkdir(exist_ok=True)
    ...
    steps=[
        ...
        ('music-scenes', """(async()=>{closeDetail();await pengpeng.changeView('audio');state.category='音乐 / 登场曲';state.query='';state.subgroup='';state.favorites=false;state.filters={};await refresh();
          if(state.total<1000||document.querySelectorAll('#audio-subgroup option').length<8)throw Error('music library incomplete');
        ...
        ('sample-accurate-preview', """(async()=>{await pengpeng.showCard('BOT_021');await pengpeng.cardTab('voices');state.pairedAudio=true;state.generalAudio=true;
          ...
          if(!audio.src.includes('audio-timelines')||audio.paused||!document.querySelector('#audio-timeline').textContent.includes('0.1 秒'))throw Error('timeline playback');
          ...
          const result={duration:audio.duration,unresolved:document.querySelectorAll('[data-timeline-play]').length,mediaClocks:1};stopPlayback();return result;})()"""),
    ]
    ...
    def wheel(mode):
        poll.stop()
        window.scrolling.stop();window.scrolling.enabled=mode != 'reduced'
        window.page.settings().setAttribute(QWebEngineSettings.WebAttribute.ScrollAnimatorEnabled,False)
        ...
        def inject(_):
            ...
            def collect():
                ...
                def done(encoded):
                    ...
                    if mode != 'reduced' and data['tailMs']>200:finish('wheel tail exceeded 200ms: '+mode);return
                    if mode=='nested' and data['outside']!=0:finish('nested scroll escaped');return
                    if mode=='reduced' and data['steps']>2:finish('reduced motion animated');return
                    ...
                    window.scrolling.enabled=True
                    window.page.settings().setAttribute(QWebEngineSettings.WebAttribute.ScrollAnimatorEnabled,False)
                    next_step();poll.start()
                window.page.runJavaScript(js,done)
            QTimer.singleShot(events[-1][0]+500,collect)
        window.page.runJavaScript(prepare,lambda _:QTimer.singleShot(100,lambda:window.page.runJavaScript('window.__start=performance.now();window.__points=[];',inject)))
